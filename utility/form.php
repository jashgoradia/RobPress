...
class Form {
	...
	public function start($options=array()) {
		...
		$_SESSION['token'] = $f3->get('CSRF'); //store CSRF token
		$hiddenField = '<input type="hidden" name="token" value="' . $_SESSION['token'] . '">'; //create a hidden field for CSRF token
		return '<form role="form" method="post" action="'.$action.'" '.$enctype.'>'. $hiddenField; //append hidden field while creating the form
	}
