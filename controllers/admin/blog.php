<?php
	...
	class Blog extends AdminController {
		...
		public function delete($f3) {
			...
			}else{ //block action if CSRF check returns false
		...
		public function add($f3) {
			if($this->request->is('post')) {
				...
				if(empty($content) || empty($summary)) {
					$errors .= '; You did not enter your content';
				}
				//date check --
				if($post->created > date('Y/m/d H:i', time())) {
					$errors .= '; Invalid date';
				}
		...
		{
			$postid = $f3->get('PARAMS.3');
			$post = $this->Model->Posts->fetchById($postid); // fetchById instead of fetch
