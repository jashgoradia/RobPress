...
class Blog extends Controller {
	...
	public function search($f3) {
		if($this->request->is('post')) {
			...
			//used named parameters while querying for search term to protect against SQL
			$ids = $this->db->connection->exec("SELECT id FROM `posts` WHERE `title` LIKE :s OR `content` LIKE :s", array(':s'=> "%$search%"));			
